...
GithubApi.prototype.getInfo = function (cb) {
  return function (next) {
    return ghRequest('/repos/' + this.owner + '/' + this.repo, cb, next)
  }
}

GithubApi.prototype.getReleases = function (cb) {
  return function (next) {
    return ghRequest('/repos/' + this.owner + '/' + this.repo + '/releases', cb, next)
  }
}

GithubApi.prototype.getContributors = function (cb) {
  return function (next) {
    return ghRequest('/repos/' + this.owner + '/' + this.repo + '/contributors', cb, next)
  }
}
